...
int main(int argc, char *argv[]) {
	...
	while(1) {
		...
		int recd_len = recv(conn_sock_fd,msg_buf,sizeof(msg_buf),MSG_DONTWAIT); // Make appropriate recv(...) call
		//printf("\nreceived request message from client:: %s \n", msg_buf);
		int kc = send(conn_sock_fd,msg_buf,strlen(msg_buf),MSG_DONTWAIT); // Make appropriate send(...) call
